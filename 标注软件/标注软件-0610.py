...
def int_to_hex_color(color_int):
    """把标注文件里的整数颜色转成 Tkinter 的 #RRGGBB 字符串"""
    try:
    ...
    # 注：若发现颜色跟标注软件里对不上，多半是 BGR 顺序，
    # 把 r 和 b 对调即可：return f"#{b:02x}{g:02x}{r:02x}"
...
class AnnotationManager:
    ...
    def __init__(self):
        ...
        self.label_to_color = {}   # name -> 整数颜色（来自标注文件）
    ...
    def load_existing(self, json_path, image_dir):
        ...
        try:
            ...
            if "images" in data and "annotations" in data and "categories" in data:
                ...
                # 新增：读取真实颜色
                self.label_to_color = {cat["name"]: cat.get("color")
    ...
    def save(self, json_path, image_dir, image_files, current_labels):
        ...
        # === 修改：确保 annotations 文件夹存在 ===
        annotations_dir = os.path.dirname(json_path)
        ...
        for idx, name in enumerate(self.labels):
            color = self.label_to_color.get(name)
            if color is None:
                # 没有记录过颜色才生成
...
class BoxDrawer:
    """框绘制管理"""
    def __init__(self, canvas, image_loader, manager=None):
        self.canvas = canvas
        self.image_loader = image_loader
        self.manager = manager          # 新增：用于查标签颜色
        self.box_rects = []
        self.box_labels = []
        self.box_label_bgs = []         # 新增：标签背景矩形 ID
        self.selected_box = None

    def _get_box_color(self, label):
        """按标签返回颜色，优先用标注文件里的真实颜色"""
        color_int = None
        if self.manager is not None:
            color_int = self.manager.label_to_color.get(label)
        if color_int is None:
            # 没有记录时按标签顺序生成一个稳定颜色
            if self.manager is not None and label in self.manager.labels:
                idx = self.manager.labels.index(label)
            else:
                idx = abs(hash(label))
            color_int = (idx * 2654435761) % 16777216
        return int_to_hex_color(color_int)
    ...
    def redraw(self, boxes):
        ...
        for bg_id in self.box_label_bgs:        # 新增：清除旧背景
        ...
        for idx, box in enumerate(boxes):
            ...
            # 1. 矩形框颜色：按标签取色；选中时加粗
            is_selected = (idx == getattr(self, 'selected_box', None))
            color = self._get_box_color(label_text)
            ...
            if bbox:
                ...
                self.canvas.tag_lower(bg_id, text_id)  # 背景压在文字下面
                self.box_label_bgs.append(bg_id)
            self.box_labels.append(text_id)
...
class FrameAnnotator:
    ...
    def change_label_color(self):
        ...
        if not label or label not in self.manager.labels:
            messagebox.showwarning("提示", "请先选择一个标签")
            return
        # 把当前颜色（整数）转成 Tk 接受的格式
        current_int = self.manager.label_to_color.get(label)
        if current_int is not None:
            # 转成 #rrggbb 作为初始颜色
            r = (current_int >> 16) & 0xFF
            g = (current_int >> 8) & 0xFF
            b = current_int & 0xFF
            init_color = f"#{r:02x}{g:02x}{b:02x}"
        else:
            init_color = "#ff0000"  # 默认红
        # 调用系统颜色选择器
        color = colorchooser.askcolor(title=f"选择 {label} 的颜色", initialcolor=init_color)
        if not color or not color[0]:
            return  # 用户取消
        # color[0] 是 (r, g, b) 元组，范围 0~255
        r, g, b = map(int, color[0])
        new_color_int = (r << 16) | (g << 8) | b
        # 更新 manager 中的颜色映射
        self.manager.label_to_color[label] = new_color_int
        # 立即重绘当前图片的框（如果当前图片有该标签的框）
        self.drawer.redraw(self.current_boxes)
        # 提示保存
        messagebox.showinfo("成功", f"标签 '{label}' 的颜色已更新，请记得 Ctrl+S 保存")
    ...
    def setup_ui(self):
        ...
        tk.Button(btn_frame1, text="🎨 颜色", command=self.change_label_color).pack(side=tk.LEFT, padx=4)
    ...
    def load_folder(self):
        ...
        # === 修改：json_path 指向 annotations 文件夹 ===
        annotations_dir = os.path.join(self.image_dir, "annotations")
        self.json_path = os.path.join(annotations_dir, "annotations.json")
        ...
        self.drawer = BoxDrawer(self.canvas, self.image_loader, self.manager)  # 传入 manager
